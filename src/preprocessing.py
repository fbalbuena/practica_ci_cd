...
class TitanicPreprocessor:
    ...
    def encode_categorical(self, df, fit=True):
            ...
            if col in df.columns:
                if fit:
                    ...
                    df[col] = df[col].apply(
                        lambda x, c=col: (
                            x
                            if x in self.label_encoders[c].classes_
                            else self.label_encoders[c].classes_[0]
                        )
                    )
        ...
        if "AgeGroup" in df.columns:
            df = pd.get_dummies(
                df, columns=["AgeGroup"], prefix="AgeGroup", drop_first=True
            )

        if "FareGroup" in df.columns:
            df = pd.get_dummies(
                df, columns=["FareGroup"], prefix="FareGroup", drop_first=True
            )
