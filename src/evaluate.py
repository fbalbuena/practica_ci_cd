...
def evaluate_model(model_filename="titanic_model_random_forest.pkl", use_test=True):
    ...
    y_proba = (
        model.predict_proba(X_eval)[:, 1] if hasattr(model, "predict_proba") else None
    )
    ...
    print(
        classification_report(y_eval, y_pred, target_names=["Not Survived", "Survived"])
    )
    ...
    if hasattr(model, "feature_importances_"):
        ...
        plot_feature_importance(
            feature_importance, reports_path / "feature_importance.png"
        )
    ...
    results = {
        ...
        "classification_report": classification_report(
            y_eval, y_pred, output_dict=True
        ),
    }
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--use-test", action="store_true", help="Use test set instead of validation set"
    )

    args = parser.parse_args()

    evaluate_model(model_filename=args.model, use_test=args.use_test)
